...
amount.oninput = () => {
  // Obtém o valor atual do input e remove os caracteres não numéricos
...
function expenseAdd(newExpense) {
  try {
    ...
    alert("Não foi possível atualizar a lista de desespesas.")
    console.log(error)
  }
}
...
function updateTotals() {
  try {
    // Recupera todos os items (li) da lista (lis).
    const items = expenseList.children

    // Atualiza a quantidade de itens da lista.
    expensesQuantity.textContent = `${items.length} ${items.length > 1 ? "despesas" : "despesa"}`

    // Variável para incrementar o totla.
    let total = 0;
